...
describe('effect', () => {
  ...
  test('effect 中监听多个响应式对象', () => {
    const observed1 = reactive({ count: 0 })
    const observed2 = reactive({ count: 0 })
    let value
    effect(() => (value = observed1.count + observed2.count))
    expect(value).toBe(0)
    observed1.count++
    expect(value).toBe(1)
    observed2.count++
  ...
  test('嵌套响应式对象', () => {
    ...
    let value
    effect(() => (value = observed.nested.count))
    expect(value).toBe(0)
  ...
  test('链式effect', () => {
    ...
    let value
    effect(() => (observed2.count = observed1.count))
    effect(() => (value = observed2.count))
